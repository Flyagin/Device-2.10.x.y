...
void make_ekran_setpoint_switch()
{
  ...
  for (unsigned int i=0; i< MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (MAX_ROW_FOR_SETPOINT_SWITCH<<1))//Множення на два константи MAX_ROW_FOR_SETPOINT_SWITCH потрібне для того, бо наодн позицію ми використовуємо два рядки (назва + значення)
    {
      if ((i & 0x1) == 0)
      {
        //У непарному номері рядку виводимо заголовок
        for (unsigned int j = 0; j<MAX_COL_LCD; j++) working_ekran[i][j] = name_string[index_language][index_of_ekran>>1][j];
        if ((index_of_ekran>>1) == INDEX_ML_STPInom)
        {
          vaga = 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для уставки номінального струму
          if (current_ekran.edition == 0) value = current_settings.setpoint_Inom; //у змінну value поміщаємо значення уставки номінального струму
          else value = edition_settings.setpoint_Inom;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPRKS_Inom)
        {
          ...
          if (current_ekran.edition == 0) value = current_settings.setpoint_r_kom_st_Inom; //у змінну value поміщаємо значення уставки ресурсу комунікаційнійої стійкості при номінальному струмі
          else value = edition_settings.setpoint_r_kom_st_Inom;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPMInom_vymk)
        {
          vaga = 10000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для уставки номінального струму вимкнення
          if (current_ekran.edition == 0) value = current_settings.setpoint_Inom_vymk; //у змінну value поміщаємо значення уставки номінального струму вимкнення
          else value = edition_settings.setpoint_Inom_vymk;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPRKS_Inom_vymk)
        {
          ...
          if (current_ekran.edition == 0) value = current_settings.setpoint_r_kom_st_Inom_vymk; //у змінну value поміщаємо значення уставки ресурсу комунікаційнійої стійкості при номінальному струмі вимкнення
          else value = edition_settings.setpoint_r_kom_st_Inom_vymk;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPPOCHATKOVYJ_RESURS)
        {
          vaga = 100000; //максимальний ваговий коефіцієнт для вилілення старшого розряду початкового ресурсу вимикача
          if (current_ekran.edition == 0) value = current_settings.setpoint_pochatkovyj_resurs; //у змінну value поміщаємо значення уставки початкового ресурсу вимикача
          else value = edition_settings.setpoint_pochatkovyj_resurs;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPKRYTYCHNYJ_RESURS)
        {
          vaga = 10000; //максимальний ваговий коефіцієнт для вилілення старшого розряду критичного ресурсу вимикача
          if (current_ekran.edition == 0) value = current_settings.setpoint_krytychnyj_resurs; //у змінну value поміщаємо значення уставки критичного ресурсу вимикача
          else value = edition_settings.setpoint_krytychnyj_resurs;
          first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
        }
        else if ((index_of_ekran>>1) == INDEX_ML_STPPOCHATKOVA_K_VYMK)
        {
          vaga = 100000; //максимальний ваговий коефіцієнт для вилілення старшого розряду ресурсу комунікаційнійої стійкості при номінальному струмі
          if (current_ekran.edition == 0) value = current_settings.setpoint_pochatkova_k_vymk; //у змінну value поміщаємо значення уставки ресурсу комунікаційнійої стійкості при номінальному струмі
        ...
        for (unsigned int j = 0; j<MAX_COL_LCD; j++)
        {
          ...
          else if ((index_of_ekran>>1) == INDEX_ML_STPMInom_vymk)
          {
            ...
            else if ((j >= (COL_SETPOINT_Inom_vymk_END + 2)) && (j <= (COL_SETPOINT_Inom_vymk_END + 3))) 
            {
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j, COL_SETPOINT_Inom_vymk_COMMA, 0);
          }
          else if ((index_of_ekran>>1) == INDEX_ML_STPRKS_Inom_vymk)
          {
...
void make_ekran_timeout_switch()
{
  ...
  for (unsigned int i=0; i< MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (MAX_ROW_FOR_TIMEOUT_SWITCH<<1))//Множення на два константи MAX_ROW_FOR_TIMEOUT_SWITCH потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      if ((i & 0x1) == 0)
      {
        //У непарному номері рядку виводимо заголовок
        for (unsigned int j = 0; j<MAX_COL_LCD; j++) working_ekran[i][j] = name_string[index_language][index_of_ekran>>1][j];
        if ((index_of_ekran>>1) == INDEX_ML_TMOON)
        {
          vaga = 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для витримки T вкл.
          if (current_ekran.edition == 0) value = current_settings.timeout_swch_on; //у змінну value поміщаємо значення витримки T вкл.
          else value = edition_settings.timeout_swch_on;
        }
        else if ((index_of_ekran>>1) == INDEX_ML_TMOOFF)
        {
          vaga = 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для витримки T відкл.
          if (current_ekran.edition == 0) value = current_settings.timeout_swch_off; //у змінну value поміщаємо значення витримки T відкл.
          else value = edition_settings.timeout_swch_off;
        }
        else  if ((index_of_ekran>>1) == INDEX_ML_TMOUDL_BLK_ON)
        {
          ...
          if (current_ekran.edition == 0) value = current_settings.timeout_swch_udl_blk_on; //у змінну value поміщаємо значення итримки T "удлинение сигнала блокировки включения"
          else value = edition_settings.timeout_swch_udl_blk_on;
        }
        else  if ((index_of_ekran>>1) == INDEX_ML_TMOPRYVODA_VV)
        {
          vaga = 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для витримки T "Привода ВВ"
          if (current_ekran.edition == 0) value = current_settings.timeout_pryvoda_VV; //у змінну value поміщаємо значення итримки T "Привода ВВ"
        ...
        for (unsigned int j = 0; j<MAX_COL_LCD; j++)
        {
          if ((index_of_ekran>>1) == INDEX_ML_TMOON)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j, COL_TMO_SWCH_ON_COMMA, 0);
          }
          else if ((index_of_ekran>>1) == INDEX_ML_TMOOFF)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j, COL_TMO_SWCH_OFF_COMMA, 0);
          }
          else if ((index_of_ekran>>1) == INDEX_ML_TMOUDL_BLK_ON)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j, COL_TMO_SWCH_UDL_BLK_ON_COMMA, 0);
          }
          else if ((index_of_ekran>>1) == INDEX_ML_TMOPRYVODA_VV)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j,COL_TMO_PRYVODA_VV_COMMA, 0);
          }
        }
      }
...
void make_ekran_control_switch()
{
  ...
  for (unsigned int i=0; i< MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (MAX_ROW_FOR_CONTROL_SWITCH<<1))//Множення на два потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      if ((i & 0x1) == 0)
      {
        //У непарному номері рядку виводимо заголовок
