...
//  unsigned int po_block_u_mtzn_x_setpoint;
  ...
  //ПО U блок. МТЗНх
  if (
      (measurement[index_IM_UAB] <= po_block_u_mtzn_x_setpoint) ||
      (measurement[index_IM_UBC] <= po_block_u_mtzn_x_setpoint) ||
      (measurement[index_IM_UCA] <= po_block_u_mtzn_x_setpoint)
     )
  {
...
// Вибір ТН1/ТН2
/*****************************************************/
uint32_t choose_tn1_tn2(unsigned int *p_active_functions)
{
  uint32_t logic_tn1_tn2_0 = 0;
  static uint32_t static_logic_tn1_tn2_0; 
  
  logic_tn1_tn2_0 |= (_CHECK_SET_BIT(p_active_functions, RANG_PUSK_K1_AVR) != 0) << 0; /* 1к+ */
...
// АВР
/*****************************************************/
void avr_handler(unsigned int *p_active_functions, unsigned int number_group_stp)
{
...
// "Перевірка фазування"
/*****************************************************/
void ctrl_phase_handler(unsigned int *p_active_functions, unsigned int number_group_stp)
{
  ...
  if ((TN1_bilshe_porogu != 0) && (TN2_bilshe_porogu != 0))
  {
    ...
    setpoint_phi = (state_delta_phi == 0) ? current_settings_prt.setpoint_ctrl_phase_phi[number_group_stp] : current_settings_prt.setpoint_ctrl_phase_phi[number_group_stp]*KOEF_POVERNENNJA_GENERAL_UP/100;
    ...
    setpoint_f = (state_delta_f == 0) ? current_settings_prt.setpoint_ctrl_phase_f[number_group_stp] : current_settings_prt.setpoint_ctrl_phase_f[number_group_stp]*KOEF_POVERNENNJA_GENERAL_UP/100;
