  ...
  else if (driver_i2c.state_execution == 1)
  {
    ...
    if (_CHECK_SET_BIT(control_i2c_taskes, TASK_READING_RTC_BIT) !=0)
    {
         ...
         )
      {
        ...
        if((state_rtc_task & STATE_FIRST_READING_RTC) == 0 ) _SET_BIT(set_diagnostyka, RTC_WORK_FIELD_NOT_SET_BIT);
      ...
      else if ((etap_reset_of_bit < ETAP_CLEAR_OF_ST_STOP) && ((read_write_i2c_buffer[0xf] & (1<<2)) != 0))
      {
        ...
        if((state_rtc_task & STATE_FIRST_READING_RTC) == 0 )
        {
      ...
      {
        //Повідомлення про цю несправність виставляємо, коли не йде тільки-но перше зчитування
        if((state_rtc_task & STATE_FIRST_READING_RTC) == 0 )_SET_BIT(set_diagnostyka, RTC_UPDATING_HALTED_BIT);
      ...
      if((state_rtc_task & STATE_FIRST_READING_RTC) !=0)
      {
        ...
        state_rtc_task &= (unsigned int)(~STATE_FIRST_READING_RTC);
      }
